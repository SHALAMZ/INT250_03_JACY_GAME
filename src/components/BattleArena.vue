...
<template>
    <div class="flex flex-col items-center w-full h-full justify-center relative z-10 p-6">
        ...
        <div class="flex flex-col md:flex-row items-center justify-center w-full max-w-6xl gap-4 md:gap-16">
        
        <!-- Player Side -->
        <div class="flex flex-col items-center animate-slide-up" style="animation-duration: 0.5s">
            <div class="relative md:w-64 md:h-64 flex items-center justify-center mb-4 md:mb-8">
            <div :class="['absolute inset-0 rounded-full blur-[50px] opacity-60 animate-pulse-glow', getElementColor(playerChoice.element)]"></div>
            <span class="text-8xl md:text-[180px] relative z-10 drop-shadow-[0_20px_40px_rgba(0,0,0,0.5)] transition-transform duration-500" :class="{'animate-shake': gameState === 'fighting', 'scale-110': gameState === 'result' && currentRoundLog?.wResult === 1}">
                {{ getWeaponIcon(playerChoice.weapon) }}
            </span>
            <span class="absolute -bottom-4 -right-4 md:bottom-0 md:right-0 text-3xl md:text-6xl bg-slate-800/80 backdrop-blur-md rounded-full p-3 border-2 border-white/20 shadow-2xl z-20 hover:scale-110 transition-transform">
        ...
        <!-- Center Results -->
        <div class="flex flex-col items-center shrink-0 w-64 md:w-80 justify-center" :class="{'opacity-0 transition-opacity': gameState === 'fighting', 'animate-pop-in': gameState === 'result'}">
            <div v-if="gameState === 'result'" class="flex flex-col items-center gap-4 w-full">
            ...
            <button @click="$emit('next')" class="w-full py-4 bg-white text-slate-900 rounded-2xl font-black md:text-xl hover:bg-gray-200 transition-all shadow-[0_0_25px_rgba(255,255,255,0.4)] hover:scale-105 active:scale-95 uppercase tracking-widest">
        ...
        <div class="flex flex-col items-center animate-slide-up" style="animation-duration: 0.5s">
            ...
            <template v-else>
                ...
                <span class="text-8xl md:text-[180px] relative z-10 drop-shadow-[0_20px_40px_rgba(0,0,0,0.5)] transition-transform duration-500" :class="{'scale-110': currentRoundLog?.wResult === -1}">
                    {{ getWeaponIcon(botChoice.weapon) }}
                </span>
                <span class="absolute -bottom-4 -left-4 md:bottom-0 md:left-0 text-3xl md:text-6xl bg-slate-800/80 backdrop-blur-md rounded-full p-3 border-2 border-white/20 shadow-2xl z-20 hover:scale-110 transition-transform">
